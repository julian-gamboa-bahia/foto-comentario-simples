...
import { GetUrlDioService } from '../../services/get-url-dio.service';
...
export class NovosComponent implements OnInit {
  ...
  constructor(
    ...
    private GetUrlDioService: GetUrlDioService
    ) {  }
...
ngOnInit() {
    ...
    this.getUrl_url_METODO();
//Para que as imagens avancem de forma automática
//this.avanco_certificados_automatico(this.indice_imagen);//Parando
}
...
/// Url da dio
  getUrl_url_METODO() {
    this.GetUrlDioService.getUrl_dio(this.etiqueta).subscribe((url_dio: String[]) => {
      this.url = url_dio.reverse();
//  console.log("  getUrl_imagem_METODO "+this.url);
    });
}
